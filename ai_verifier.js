...
async function verifyDNA(mappedData, screenshotPath, logoPath, youtubeData = null) {
    ...
    try {
        ...
        const prompt = `
            ...
            6. Generate a concise, engaging 'website_summary' of the company based purely on the website JSON provided. If the website JSON lacks sufficient textual context or is missing, DO NOT output an empty string. Instead output exactly: "The extracted website data lacked enough readable HTML text paragraphs for the AI to construct a meaningful mission summary."
            ${youtubeData ? `
            7. From the YouTube Description provided above, extract any Social Media profile links (Facebook, Twitter/X, Instagram, TikTok, LinkedIn, Pinterest, etc.) and add their raw URLs as an array of strings to the 'youtube_social_links' key.
            8. From the YouTube Description provided above, look for any OTHER URLs, links, or Calls to Action (CTAs) that are NOT social media profile links (e.g. articles, stores, merchandise, Patreon, reports). Do NOT duplicate any link that you already placed in 'youtube_social_links'. For each non-social link, use your AI reasoning to create a highly compelling, clickable 'button_name' (strictly MAXIMUM 30 characters) written in the bold, minimalist, heavily-curated marketing voice of Steve Jobs (e.g. "Experience the Magic", "Read the Report"). Combine this button name with the absolute 'url' and the original 'context' into a JSON object, and add them all to the 'youtube_ctas' array.
