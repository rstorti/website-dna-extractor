...
async function assertRuntimeReadiness(env, getSupabase, logger = console) {
  ...
  if (missing.length > 0) {
    throw new Error(`Refusing to start: missing required environment variables: ${missing.join(', ')}`);
  }
  ...
  if (historyProbe.error) {
    throw new Error(`Refusing to start: Supabase extraction_history schema check failed: ${historyProbe.error.message}`);
  }

  if (jobsProbe.error) {
    throw new Error(`Refusing to start: Supabase ${JOB_TABLE} schema check failed: ${jobsProbe.error.message}`);
  }
