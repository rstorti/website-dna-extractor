...
function getMissingEnvVars(env) {
  ...
  if (env.NODE_ENV === 'production') {
    if (!env.JOB_API_KEY && !env.CODEX_JOB_API_KEY) {
      missing.push('JOB_API_KEY or CODEX_JOB_API_KEY');
    }
    if (!env.ADMIN_LOGIN_KEY && !env.JOB_API_KEY) {
      missing.push('ADMIN_LOGIN_KEY or JOB_API_KEY');
    }
...
async function assertRuntimeReadiness(env, getSupabase, logger = console) {
  ...
  logger.log('[BOOT] Probing Supabase schema — extraction_history + extraction_jobs + campaign_sessions ...');

  const [historyProbe, jobsProbe, sessionsProbe] = await Promise.all([
    ...
    supabase.from('campaign_sessions').select('tenant_id', { head: true, count: 'exact' }).limit(1),
  ]);
  ...
  logger.log('[BOOT] campaign_sessions probe:', sessionsProbe.error ? `FAIL — ${formatSupabaseError(sessionsProbe.error)}` : 'OK');
  ...
  if (sessionsProbe.error) {
    throw new Error(
      `Refusing to start: Supabase campaign_sessions schema check failed: ${formatSupabaseError(sessionsProbe.error)}`
    );
  }
