...
class JobStore {
  ...
  async createJob({
    ...
    if (!effectiveTenantId || (isProd && effectiveTenantId === 'default')) {
      throw new Error('Valid tenant ID is required (default tenant is not permitted in production mode)');
    }
  ...
  async getJob(jobId, { tenantId = null } = {}) {
    ...
    const job = this.memoryJobs.get(jobId);
    if (!job) return null;
    if (job.tenant_id !== effectiveTenantId) return null;
