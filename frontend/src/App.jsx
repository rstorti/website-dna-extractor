...
function App() {
    ...
    const [summaryText, setSummaryText] = useState({ website: '', youtube: '', combined: '', linkedin: '' });
    ...
    const [docExtractionResult, setDocExtractionResult] = useState(null);
    const [docExtractionError, setDocExtractionError] = useState(null);
    ...
    const handleDocumentUpload = async (files) => {
        ...
        const validFiles = fileArray.filter(f => allowed.some(ext => f.name.toLowerCase().endsWith(ext)));
        if (validFiles.length === 0) {
            showToast('⚠️ Unsupported file type. Please upload PDF, TXT, or MD files.', 'warning', 5000);
            return;
        }
        setUploadedFiles(validFiles);
        setDocExtractionError(null);
        setDocExtractionResult(null);
        setIsDocExtracting(true);

        const formData = new FormData();
        validFiles.forEach(f => formData.append('documents', f));
        if (url) formData.append('brandWebsiteUrl', url.trim());

        try {
            const res = await fetch(`${API_BASE_URL}/api/extract-document`, {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + getSessionToken() },
                body: formData,
            });
            ...
            setDocExtractionResult(data);
            showToast(`✅ Extracted ${data.entities?.length || 0} entities from ${data.sourceFiles?.length || 0} file(s)`, 'success', 5000);
        } catch (err) {
            console.error('Document extraction failed:', err);
            setDocExtractionError(err.message);
            showToast(`⚠️ Document extraction failed: ${err.message}`, 'warning', 6000);
        } finally {
            setIsDocExtracting(false);
        }
    };
    ...
    const handleExtract = async () => {
        ...
        setSummaryText({ website: '', youtube: '', combined: '', linkedin: '' });
        setDash2SelectedImages([]);
        ...
        try {
            ...
            setSummaryText({
                ...
                linkedin: data.linkedinData?.description || data.linkedinData?.headline || '',
            ...
            const featured = [
                ...
                ...(data.linkedinData?.avatarUrl ? [data.linkedinData.avatarUrl] : [])
            ];
                    ...
                    <>
                        <div className="input-card">
                            ...
                            <div className="search-bar" style={{ display: 'flex', gap: '1.5rem', maxWidth: '850px', margin: '0 auto', alignItems: 'stretch' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 0, marginLeft: '2rem', gap: '0.8rem', alignItems: 'center' }}>
                                    ...
                                    <div
                                        ...
                                        style={{
                                            ...
                                            background: isDragOver ? 'rgba(249,157,50,0.08)' : 'rgba(0,0,0,0.2)',
                                            minWidth: '180px',
                                            maxWidth: '220px',
                                        }}
                                    >
                                        ...
                                        />
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={isDragOver ? 'var(--primary)' : 'var(--text-secondary)'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: '0.3rem' }}>
                                            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                                            <polyline points="17 8 12 3 7 8"/>
                                            <line x1="12" y1="3" x2="12" y2="15"/>
                                        </svg>
                                        <p style={{ margin: 0, color: isDragOver ? 'var(--primary)' : 'var(--text-primary)', fontWeight: '600', fontSize: '0.78rem' }}>
                                            {isDocExtracting ? 'Extracting...' : 'Upload Docs'}
                                        </p>
                                        <p style={{ margin: '0.15rem 0 0', color: 'var(--text-secondary)', fontSize: '0.65rem' }}>
                                            PDF, TXT, MD
                                        </p>
                                        {isDocExtracting && <div className="loader" style={{ width: '16px', height: '16px', borderWidth: '2px', margin: '0.3rem auto 0' }}></div>}
                                        {uploadedFiles.length > 0 && !isDocExtracting && (
                                            <div style={{ marginTop: '0.4rem' }}>
                                                {uploadedFiles.map((f, i) => (
                                                    <span key={i} style={{ display: 'block', background: 'rgba(249,157,50,0.15)', color: 'var(--primary)', padding: '0.15rem 0.4rem', borderRadius: '4px', fontSize: '0.65rem', fontWeight: '500', marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        📄 {f.name}
                                                    </span>
                                                ))}
                                            </div>
                            ...
                            {docExtractionError && (
                                <div style={{ marginTop: '1rem', padding: '0.8rem 1rem', background: 'rgba(255,68,68,0.1)', border: '1px solid rgba(255,68,68,0.3)', borderRadius: '8px', color: '#ff6b6b', fontSize: '0.85rem' }}>
                                    ⚠️ {docExtractionError}
                                </div>
                            )}

                            {/* Document Extraction Results */}
                            {docExtractionResult && (
                                <div style={{ marginTop: '1.2rem', padding: '1.2rem 1.5rem', background: 'rgba(76,175,80,0.06)', border: '1px solid rgba(76,175,80,0.2)', borderRadius: '12px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.8rem' }}>
                                        <span style={{ fontSize: '1.2rem' }}>📋</span>
                                        <div>
                                            <span style={{ fontWeight: '700', color: 'var(--text-primary)', fontSize: '1rem' }}>{docExtractionResult.documentTitle}</span>
                                            <span style={{ marginLeft: '0.6rem', padding: '0.15rem 0.5rem', background: 'rgba(249,157,50,0.2)', color: 'var(--primary)', borderRadius: '4px', fontSize: '0.72rem', fontWeight: '600', textTransform: 'uppercase' }}>
                                                {docExtractionResult.documentType?.replace(/_/g, ' ')}
                                            </span>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '0.8rem' }}>
                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                                            🔍 <strong>{docExtractionResult.entities?.length || 0}</strong> entities found
                                        </span>
                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                                            📂 <strong>{docExtractionResult.suggestedGroups?.length || 0}</strong> groups
                                        </span>
                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.82rem' }}>
                                            ⏱️ {((docExtractionResult.totalMs || 0) / 1000).toFixed(1)}s
                                        </span>
                                    </div>
                                    {/* Group Summary Chips */}
                                    {docExtractionResult.suggestedGroups?.length > 0 && (
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.8rem' }}>
                                            {docExtractionResult.suggestedGroups.map((g, i) => (
                                                <span key={i} style={{ padding: '0.2rem 0.6rem', background: 'rgba(33,150,243,0.15)', color: '#64b5f6', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '500' }}>
                                                    {g.name} ({g.entityCount})
                                                </span>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <details style={{ marginTop: '0.5rem' }}>
                                        <summary style={{ cursor: 'pointer', color: 'var(--primary)', fontSize: '0.85rem', fontWeight: '600', userSelect: 'none' }}>
                                            View extracted entities →
                                        </summary>
                                        <div style={{ marginTop: '0.6rem', maxHeight: '350px', overflowY: 'auto', paddingRight: '0.5rem' }}>
                                            {docExtractionResult.entities?.map((entity, idx) => (
                                                <div key={idx} style={{
                                                    padding: '0.6rem 0.8rem',
                                                    marginBottom: '0.4rem',
                                                    background: 'rgba(0,0,0,0.25)',
                                                    borderRadius: '8px',
                                                    borderLeft: `3px solid ${entity.entityType === 'person' ? '#64b5f6' : entity.entityType === 'place' ? '#81c784' : '#ffb74d'}`,
                                                }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                                                        <span style={{ fontSize: '0.9rem' }}>
                                                            {entity.entityType === 'person' ? '👤' : entity.entityType === 'place' ? '📍' : entity.entityType === 'organization' ? '🏢' : '📦'}
                                                        </span>
                                                        <strong style={{ color: 'var(--text-primary)', fontSize: '0.88rem' }}>{entity.name}</strong>
                                                        {entity.role && <span style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>— {entity.role}</span>}
                                                    </div>
                                                    <p style={{ margin: '0.2rem 0 0', color: 'var(--text-secondary)', fontSize: '0.78rem', lineHeight: 1.4 }}>
                                                        {entity.description?.substring(0, 200)}{entity.description?.length > 200 ? '...' : ''}
                                                    </p>
                                                    <div style={{ marginTop: '0.3rem', display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                                                        <span style={{ padding: '0.1rem 0.4rem', background: 'rgba(255,255,255,0.06)', borderRadius: '4px', fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
                                                            {entity.groupSuggestion}
                                                        </span>
                                                        {entity.ctaSuggestion && (
                                                            <span style={{ padding: '0.1rem 0.4rem', background: 'rgba(249,157,50,0.12)', borderRadius: '4px', fontSize: '0.7rem', color: 'var(--primary)' }}>
                                                                {entity.ctaSuggestion}
                                                            </span>
                                                        )}
                                                    </div>
                                    ...
                                    <button
                                        onClick={() => {
                                            navigator.clipboard.writeText(JSON.stringify(docExtractionResult, null, 2));
                                            showToast('✅ Extraction JSON copied to clipboard!', 'success', 3000);
                                        }}
                                        style={{
                                            marginTop: '0.8rem',
                                            padding: '0.5rem 1rem',
                                        ...
                                        onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(249,157,50,0.25)'}
                                        onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(249,157,50,0.15)'}
                                    >
                                        📋 Copy Full JSON
                                    </button>
                                </div>
                            ...
                            <div className="dashboard-grid" style={{ '--active-select': showJsonPreview ? '#4caf50' : 'var(--primary)' }}>
                                ...
                                <div className="glass-panel top-header" style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
                                    ...
                                    <div className="header-descriptions" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                        ...
                                        {['website', 'combined', 'youtube', 'linkedin'].map(type => {
                                            if (!summaryText[type] && type !== 'website') return null; // Don't show combined, youtube, or linkedin if it doesn't exist
                                            
                                            // Fallback label names
                                            const labels = { website: 'Website Summary', combined: 'Combined Summary', youtube: 'YouTube Video Summary (AI Generated)', linkedin: 'LinkedIn Profile Summary' };
                                ...
                                {/* 2.5 LinkedIn Profile Data */}
                                {result.linkedinData && (
                                    <div className="glass-panel" style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', marginBottom: '2rem' }}>
                                        {result.linkedinData.avatarUrl && (
                                            <img src={result.linkedinData.avatarUrl} alt="LinkedIn Avatar" style={{ width: '100px', height: '100px', borderRadius: '50%', border: '2px solid var(--primary)', objectFit: 'cover' }} />
                                        )}
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                                            <h3 style={{ margin: 0, color: 'var(--primary)', fontSize: '1.1rem' }}>LinkedIn Profile Scraped</h3>
                                            {result.linkedinData.name && <strong style={{ fontSize: '1.2rem' }}>{result.linkedinData.name}</strong>}
                                            {result.linkedinData.headline && <span style={{ color: 'var(--text-secondary)' }}>{result.linkedinData.headline}</span>}
                                            {result.linkedinData.description && <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.9rem', color: 'rgba(255,255,255,0.7)', fontStyle: 'italic' }}>"{result.linkedinData.description.slice(0, 150)}{result.linkedinData.description.length > 150 ? '...' : ''}"</p>}
                                        </div>
                                    </div>
                                )}
